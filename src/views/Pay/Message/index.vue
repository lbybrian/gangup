	...
	<div id="login_box">
		...
		<div class="main_box">
			...
			<div class="show_box" v-for="item in mainlist">
				<p>
					<h3>{{item.content}}{{item.hehe}}</h3>{{item.username}}{{item.csdata}}<b>{{item.time}}</b>
				</p>
			...
			<div class="page" v-show="mainlist.length>=1">
				...
				<a v-for="item in pageSum" href="javascript:;" @click="getDb(item)" :style="activeBK(item)">{{item}}</a>
			</div>
		</div>
...
export default {
  ...
  methods: {
    ...
    addOne() {
      this.$axios
        .post("http://127.0.0.1:8090/addnews", {
          dbname: this.dbname,
          jhname: this.jhname,
    ...
    async getDb(pageIndex = 1) {
      await this.$axios
        .get(
          "http://127.00.0.1:8090/getdb?PageIndex=" + pageIndex
          //						{params:{
    ...
    async deleteOne(id) {
      await this.$axios
        .delete("http://127.0.0.1:8090/delete/" + id)
        .then(({ data }) => {
          console.log(data);
          if (data.ok === 1) {
