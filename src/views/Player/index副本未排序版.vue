    ...
    <el-row :gutter="20">
      <el-col :span="18">
        <div class="allname-box">
        	<el-button @click="rank">重新排列</el-button>
      ...
      getImgsList() {
        ...
        this.$axios.get("http://127.0.0.1:8090/getimgslist").then((res) => {
          ...
          if (res.status === 200) {
            ...
            // 				console.log(this.imgUrls)
            // 图片数组排序
            // this.imgUrls.slice(0,11)
            // let brr = [];
            this.imgUrls.forEach((item) => {
//          	console.log(item)
              // console.log(item.url.match(/_(\S*)\./)[1]);//截取‘_’和‘.’(之间的值最后一次出现‘.’才返回)
                 console.log(item.url.match(/_(\S+?)\./)[1]);//截取‘_’和第一个‘.’之间的值
              // let aa =item.url.slice(16,18)
              this.brr.push(parseInt(item.url.match(/_(\S+?)\./)[1]));
            });
            	
            	this.rank2();
            // console.log(this.brr)
            // for (let i=1;i<=this.imgUrls.length-1;i++){//重在体现循环次数（前两位不用比较-->所以比较次数是length-1）
            //             for (let j=1;j<=this.imgUrls.length-i-1;j++){//如下图（j=1是下图的1，arr.length-i是下图循环次数的表达式）
            //             // console.log(this.imgUrls[j-1].url.match(/_(\S+?)\./)[1],'~~~~~~~~~~',this.imgUrls[j].url.match(/_(\S+?)\./)[1])
            //                 if(this.imgUrls[j-1].url.match(/_(\S+?)\./)[1]>this.imgUrls[j].url.match(/_(\S+?)\./)[1]){//交换逻辑
            //                     let sub=this.imgUrls[j-1];
            //                     this.imgUrls[j-1]=this.imgUrls[j];
            //                     this.imgUrls[j]=sub;
            //                 }
            //             }
            //         }
      ...
      rank2() {//异步执行瞬间获取排序结果
       console.log('rank2',this.brr)
        // let ar=[1,11,2,102,4,10,100,3,101,5,103];
        // let ar=this.brr;
        var i = 0,j;
      ...
      //               // 内层循环控制每轮比较几个元素
      //                 console.log(parseInt(arr[j]) , parseInt(arr[j+1]))
