		...
		<el-row v-if="doSha">
			<el-col :span="3">
				<div class="typeList">
					...
					<h1>Drill</h1>
					<!--<h2>Drill</h2>-->
					<div v-for="item in typeList" :class="{typeItem:true,active:params.objType===item.val}" @click="changeType(item)">
						{{item.title}}
					</div>
				</div>
			</el-col>
				...
				<div class="r">
					...
					<div>
						<el-table :data="tableData">
							...
							<el-table-column prop="data" label="操作" solt="operation" width="80">
		...
		<el-row v-else>
			<el-table :data="dataList" v-if="dataList.length>0">
				...
				<el-table-column label="任务名称" width="180">
					<template slot-scope="scope">
						<el-select v-model="scope.row.rwmc" multiple filterable remote reserve-keyword placeholder="请输入关键词" :remote-method="remoteMethod" multiple allow-create default-first-option>
							<el-option>
