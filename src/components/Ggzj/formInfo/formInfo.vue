          ...
          <slot v-if="!!item.items && !!item.type">
            ...
            >
              <slot>
                ...
                <!-- <span v-else-if="d.type === 'button'||item.type==='button'" v-for="btn in d.value"> -->
                <span v-else-if="d.type === 'button'||item.type==='button'">
                  <span>
                    <el-button   v-if="d.type==='button1'" @click="submitForm('dataInfo')" >
                      {{d.name}}
                    </el-button>
                  </span>

                  <span>
                    <el-button  v-if="d.type==='button2'" @click="emptyFormValue(dataInfo.data)" >
								  <!--<el-button   v-if="btn.type==='button2'" @click="resetForm('dataInfo')" >-->
								    	{{d.name}}
								   </el-button>
                  </span>
                </span>
  ...
  methods: {
    ...
    emptyFormValue(data) {
        for (var i = 0; i < data.length; i++) {
          if (data[i].type === "checkbox") {
            for(var j = 0; j < data[i].items.length; j++){
              data[i].items[j].checked = false;
            }
            data[i].checkAll = false;
            data[i].isIndeterminate = false;
            // this.handleCheckAllChange(data[i])
          }
          else if(data[i].type === "select"){
            for(var j = 0; j < data[i].items.length; j++){
              console.log(data[i].items[j])
              data[i].items[j].value = '';
            }
          }
          else{
              data[i].value = '';
    ...
    handleCheckedChange(item, index) {
      ...
      console.log('aaaaaaaaaaa',item.checkAll)
